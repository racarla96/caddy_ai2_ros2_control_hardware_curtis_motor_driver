...
echo
echo "Introduce el número de serie (por ejemplo, HW033114):"
read HWSERIAL

INTERFACE_NAME="can_motor_drv"

# Crear la regla udev
RULE1="SUBSYSTEM==\"net\", ACTION==\"add\", ATTRS{serial}==\"$HWSERIAL\", NAME=\"$INTERFACE_NAME\", RUN+=\"/bin/sh -c 'sleep 1 && /sbin/ip link set $INTERFACE_NAME type can bitrate 125000 && /sbin/ip link set up $INTERFACE_NAME'\""

echo -e "\nLa siguiente regla será añadida a /etc/udev/rules.d/80-can-usb.rules:"
echo $RULE1
...
# Escribe la regla
echo $RULE1 | sudo tee /etc/udev/rules.d/80-can-usb.rules
