...
// Obtener número de frames disponibles para lectura
int SocketCANInterface::getAvailableFrameCount() {
  if (!initialized_ || socket_fd_ < 0) {
    return -1;
  }
  
  int count = 0;
  if (ioctl(socket_fd_, FIONREAD, &count) < 0) {
    std::cerr << "Error al obtener número de bytes disponibles: " << strerror(errno) << std::endl;
    return -1;
  }
  
  // Convertir bytes a número de frames
  return count / sizeof(struct can_frame);
}
